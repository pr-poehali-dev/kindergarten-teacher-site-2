...
export default function Index() {
  ...
  const [reviews, setReviews] = useState<{id:number,name:string,text:string,rating:number,created_at:string}[]>([]);
  ...
  useEffect(() => {
    fetch(REVIEWS_URL)
      .then(r => r.json())
      .then(d => { try { setReviews(JSON.parse(d).reviews || []); } catch { setReviews(d.reviews || []); } })
      .catch(() => {});
  }, []);
            ...
            <div className="bg-kidz-cream rounded-3xl p-6">
              ...
              {reviewSent ? (
                <div className="text-center py-8">
                  ...
                  <button onClick={() => setReviewSent(false)} className="mt-4 text-kidz-orange text-sm font-semibold underline">Написать ещё</button>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {[1,2,3,4,5].map(s => (
                      <button key={s} onClick={() => setReviewForm(p => ({...p, rating: s}))} className={`text-2xl transition-transform hover:scale-110 ${s <= reviewForm.rating ? 'text-yellow-400' : 'text-gray-300'}`}>★</button>
                    ))}
                  </div>
                  <button
